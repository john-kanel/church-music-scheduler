...
function buildEventDescription(event: EventWithDetails): string {
  ...
  lines.push(`⏰ Start: ${event.startTime.toLocaleString()}`)
  if (event.endTime) {
    lines.push(`⏰ End: ${event.endTime.toLocaleString()}`)
  }
  
  // Add live sync information for debugging
  lines.push('')
  lines.push(`🔄 Last Updated: ${new Date().toLocaleString()}`)
  lines.push('📱 This is a LIVE calendar - updates automatically')
  
  // Add Church Music Pro branding
  lines.push('')
  lines.push('Generated by Church Music Pro')
  lines.push('https://churchmusicpro.com')
...
/**
 * Formats a date for iCal (UTC format)
 */
